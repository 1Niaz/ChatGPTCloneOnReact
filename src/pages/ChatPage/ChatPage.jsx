...
import ChatGptIcon from '../../img/Ellipse 1.svg';
...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    const getAnswer = async () => {
      await Typing("Hello! How can I assist you today?", 'CHAT GPT');
    };
  ...
  const handleSendMessage = async () => {
    ...
    setMessages((prev) => [...prev, { text: inputMessage, sender: 'YOU', key: Date.now() }]);
    ...
    try {
      ...
      if (apiResponse && apiResponse.message && apiResponse.message.trim()) {
        setMessages((prev) => [
          ...prev,
          { text: apiResponse.message.trim(), sender: 'CHAT GPT', key: Date.now() },
        ]);
      }
    } catch (error) {
      console.error("Poshel nahui ", error);
        ...
        <div className="container">
          {messages && messages.map((message, index) => (
            (message.text && message.text.trim()) && (
              <div key={message.key} className={message.sender === 'YOU' ? 'you' : 'gpt'}>
                {message.sender === 'YOU' && (
                ...
                {message.sender === 'CHAT GPT' && (
                  <div className="chat-nick">
                    <div className="img">
                      <img src={ChatGptIcon} alt="" />
                    </div>
                    <div className="title">CHAT GPT</div>
                  </div>
                )}
                {message.text && message.text.trim()}
              </div>
